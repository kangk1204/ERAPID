    ...
    if (tolower(batch_method) == "sva") {{
      ...
      try({{
        ...
        if (nsv > 0) {{
          ...
          # Assess association of SVs with group and known covariates
          assoc <- data.frame(SV=paste0("SV", seq_len(nsv)), p_group=NA_real_)
          for (i in seq_len(nsv)) {{
            sv <- svseq$sv[, i]
            # group_col is a factor
          ...
          # Also compute min p-value across covariates (if any)
          if (length(batch_cols_vec) > 0) {{
            for (vn in batch_cols_vec) {{
              pcol <- paste0("p_", vn)
              assoc[[pcol]] <- NA_real_
          ...
          # Use the configured threshold (default 0.05) so that if any SV
          # associates with the main group (risk of over-correction),
          # we keep a design-only model.
          ...
          if (is.finite(min_p_group) && min_p_group >= sva_corr_p_thresh) {{
            choice <- "sva"
            # Attach SVs to coldata and rebuild dds
            for (i in seq_len(nsv)) coldata[[paste0("SV", i)]] <- svseq$sv[, i]
            sv_terms <- paste(paste0("SV", seq_len(nsv)), collapse = " + ")
            new_design <- if (nzchar(cov_terms)) paste0("~ ", sv_terms, " + ", cov_terms, " + ", group_col) else paste0("~ ", sv_terms, " + ", group_col)
            ...
            # Heatmap and PCA (AUTO) for reporting in supplementary figures

# Derive assoc_mat (SV x covariates p-values) for heatmap if possible
if (exists('assoc') && is.data.frame(assoc)) {
  ...
  if (length(cov_cols) > 0) {
    ...
    colnames(assoc_mat) <- sub('^p_', '', colnames(assoc_mat))
          ...
          html <- paste0(
            ...
            "<li><b>Assoc TSV:</b> ", if (file.exists(out_assoc)) paste0("<a href='", basename(out_assoc), "'>", basename(out_assoc), "</a>") else "(not written)", "</li>",
            ...
            "<p style='color:#666;margin-top:10px'>AUTO rule: if any SV associates with group at p < ", esc(sva_corr_p_thresh), ", keep design-only; otherwise include SVs.</p>",
            "</body></html>"
          )
          ...
          writeLines("design", file.path(outdir, paste0(gse_id, "__", group_col, "__auto_choice.txt")))
        }}
      }}, silent=TRUE)
