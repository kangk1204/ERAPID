  ...
  if (any(na_mask)) df$Symbol[na_mask] <- df$GeneID[na_mask]
      ...
      save_interactive_table <- function(df_in, out_html, title_str, page_size=20) {
        ...
        html <- paste0(
          ...
          'const lines=dataTSV.split(LF).map(l=>l.replaceAll(CR,"")).filter(l=>l.length>0);',
          'const header=lines[0].split(TAB);',
          'const rows=lines.slice(1).map(l=>l.split(TAB));',
          ...
          'function escCell(s){return String(s ?? "").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;");}',
          ...
          'function filtered(){if(!query) return rows; const q=query.toLowerCase(); return rows.filter(r=>r.join(TAB).toLowerCase().includes(q));}',
          'function render(){let data=filtered(); if(sortCol>=0) data=[...data].sort(comparator(sortCol)); const n=data.length; const pages=Math.max(1,Math.ceil(n/pageSize)); if(page>=pages) page=pages-1; const start=page*pageSize; const end=Math.min(start+pageSize,n); let html=""; for(let i=start;i<end;i++){const r=data[i]; html+="<tr>"+r.map(c=>"<td>"+escCell(c)+"</td>").join("")+"</tr>";} tBody.innerHTML=html; pinfo.textContent=(n+" rows · page "+(page+1)+"/"+pages); prev.disabled=(page<=0); next.disabled=(page>=pages-1);}',
